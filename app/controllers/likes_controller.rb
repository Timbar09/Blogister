class LikesController < ApplicationController
  ...
  def create
    ...
    if @like
      if @like.destroy
        redirect_back(
          fallback_location: user_post_path(@post.author, @post),
          notice: 'You successfully unliked the post.'
        )
      else
        redirect_back(
          fallback_location: user_post_path(@post.author, @post),
          alert: 'Error: Could not unlike the post.'
        )
      end
    else
      ...
      if @like.save
        redirect_back(fallback_location: user_post_path(@post.author, @post),
                      notice: 'You successfully liked the post.')
      else
